<div class="row">
    <div class="medium-6 columns">
        ...
        {{ Form::select('academic_session_id', $academicSessions, $resultConfiguration->academic_session_id, ['class' => $errors->has('academic_session_id')?'error':'']) }}
...
<div class="row">
    <div class="medium-6 columns">
        <fieldset>
            ...
            @foreach($resultConfiguration->assessments as $assessment)

            @if($assessment->assessment)
            {{ Form::label('assessment_' . $assessment->assessment->id, "{$assessment->assessment->short_name} Weightage", ['class' => ($errors->has('assessment.' . $assessment->assessment->id)?'error':'')]) }}

            <div class="row collapse">
                <div class="small-10 medium-10 columns">
                    {{ Form::text("assessment[{$assessment->assessment->id}]", $assessment->weightage, ['class' => $errors->has('assessment.' . $assessment->assessment->id)?'error':'', 'id' => 'assessment_' . $assessment->assessment->id]) }}
                </div>
                <div class="small-2 medium-2 columns">
                    <span class="postfix"> / 100</span>
                </div>
            </div>
            @if($errors->has('assessment.' . $assessment->assessment->id))
            <small class="error">{{ $errors->first('assessment.' . $assessment->assessment->id) }}</small>
            @endif
